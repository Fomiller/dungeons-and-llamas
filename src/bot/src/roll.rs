...
pub fn roll_command(cmd: CommandInteraction) -> CreateInteractionResponse {
    let count = get_count(&cmd);

    let sides = get_sides(&cmd);
    ...
    let content = format!(
        "Roll: {}\nYou rolled {}!",
        create_roll_text(dice_values, modifier),
        roll
    );
