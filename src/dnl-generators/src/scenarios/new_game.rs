...
pub static NEW_GAME_SYSTEM_PROMPT: &str = "
...
pub static NEW_GAME_JSON_PROMPT: &str = "
Using the context provided use the 'NewGame' tool to create a level {{1}} new game scenario.
The responses should be json with the following structure:
{{example}}
";

pub static NEW_GAME_TEXT_PROMPT: &str = "
Create a random {{theme}} starting scenario for the player to begin their adventure with. Make sure that the scenario is truely unique
to the previous examples provided in the context if there are any. The player constantly wants to feel like they are 
being presented with brand new scenarios every time. The scenario needs to end with giving the player the option to choose from 
an assortment of weapons to help them on their adventure.
";
